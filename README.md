...
- Cover the basic CRUD functionality without writing much boilerplate code.
...
- In most cases, you may not need redux at all. I'd suggest trying [React Query](https://tanstack.com/query/v3/) with [React Context](https://react.dev/learn/passing-data-deeply-with-context) instead.
- If you use redux to store (cache) and get access to data from your Backend, use [React Query](https://tanstack.com/query/v3/). 
- If you use redux to manipulate the state of your App and teleport data deep inside of your app, use the built-in React tool [React Context](https://react.dev/learn/passing-data-deeply-with-context).
- It's highly opinionated library, hard to customize compared to other fetching libraries.
...
## How to work with CRUD using Redux Toolkit RTK Query
Keep in mind that some of the URLs here are protected, and we pass our access tokens there. We'll discuss how to accomplish this later here.
### Fetch data
When fetching data in your API layer, define query and tags like:
```
...
```
- `query` is used by your app to modify URL or params you're passing to your data fetching function (`baseQuery` in `redux/api/baseApi`). It can return `string`, `object`. For more information [read the RTK Query docs.](https://redux-toolkit.js.org/rtk-query/usage/queries). 
- `providesTags` is helpful information allowing RTK Query to understand how to store (cache) data. Pay attention to `providesTags: result => [{ type: 'Post', id: 'postId:${result?.id}' }]` where we use dynamic tags. If we update data by executing `CREATE`, `DELETE`, or `UPDATE`, we may provide invalidatesTags there, so RTK Query will know that the stored (cached) data isn't valid anymore and we need to refetch data. We'll see it later. [Read this for more information.](https://redux-toolkit.js.org/rtk-query/usage/automated-refetching)
...
What's good about that hooks:
- We don't have to call api from `useEffect` anymore and we don't have to handle the API state in our app.
- We don't have to worry about multiple components running the same the same call. Only one networking call will be executed. A new call will be fired when stored (cached data) will be invalidated.
...
## Some usefull topics:
- Usefull kitchen example [here](https://codesandbox.io/p/sandbox/github/reduxjs/redux-toolkit/tree/master/examples/query/react/kitchen-sink?file=%2Fsrc%2Fapp%2Fservices%2Fposts.ts&from-embed=). The example may be too big and overwhelming. So I made a simpler version with explanation of our flow.
